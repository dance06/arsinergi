<?php
    ...
    if($del != '')
    {
        mysqli_query($conn, 'DELETE FROM master_lainlain WHERE id = '.$del.'');
    }
    ...
    if($_POST['save'] == 1)
    {
        ...
        mysqli_query($conn, 'DELETE FROM master_lainlain WHERE id_project = '.$id_project.'');
        ...
        for($i=1; $i<=90; $i++)
        {
            ...
            $gol = $_POST['gol_'.$i];
            $item = $_POST['item_'.$i];
            $keterangan1 = $_POST['keterangan1_'.$i];
            $qty = $_POST['qty_'.$i];
            $sat = $_POST['sat_'.$i];
            ...
            $keterangan2 = $_POST['keterangan2_'.$i];
            ...
            {
                $sql = 'INSERT INTO master_lainlain (
                id_project,
                project_count,
                tahun,
                periode,
        ...
        header("location: lain_lain.php?edit=".$id_project."&scs=1");
    }
        ...
        <script>
            ...
            function total(a)
            {
                ...
                for(i=1; i<=90; i++)
                {
                    ...
                    var res = jml.replace(",", "");
                    var total = qty*res;
                    document.getElementById('jml_'+i).value = total.toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,');
                }
            }
        </script>
        ...
        <div style="position:relative; padding:20px">
            ...
            ?>
                <form method="post" action="lain_lain.php">
                    <div style="position:relative; margin-top:0px; width:1720px">

                        <a href="lain_lain.php">
                        ...
                        <div style="position:relative; margin-bottom:20px; margin-top:20px; font-weight:bold; font-size:20px">
                            LAIN LAIN - <?php echo $prj['project_name'];?>
                        </div>
                        ...
                        <div style="position:relative; margin-bottom:10px">
                            ...
                            <div class="table_header" style="width:200px;">JUMLAH</div>
                            <div class="table_header" style="width:200px;">KETERANGAN</div>
                        ...
                        for($k=1; $k<=90; $k++)
                        {
                            ...
                            $check_total = mysqli_num_rows(mysqli_query($conn, 'SELECT * FROM master_lainlain
                            ...
                            if($check_total == 0)
                            {
                                if($k == 1)
                                {
                                    $total = mysqli_num_rows(mysqli_query($conn, 'SELECT * FROM master_lainlain
                                    WHERE id_project = '.$edit.'
                                    AND project_count = '.$k.''));


                                    $total_next = mysqli_num_rows(mysqli_query($conn, 'SELECT * FROM master_lainlain
                                    WHERE id_project = '.$edit.'
                            ...
                            <div id="upah_<?php echo $k; ?>" <?php if($total == 0) echo 'style="visibility:hidden; display:none; height:0px; margin-bottom:20px"'; else echo 'style="margin-bottom:20px"'; ?>>
    							...
    							<?php
                                    $g = mysqli_fetch_array(mysqli_query($conn, 'SELECT * FROM master_lainlain
                                    WHERE id_project = '.$edit.'
                                    AND project_count = '.$k.''));
                                    ...
                                    $keterangan1 = str_replace("##", '"', $g['keterangan1']);
                                    $keterangan1 = str_replace("#", "'",  $keterangan1);

                                    $keterangan2 = str_replace("##", '"', $g['keterangan2']);
                                ...
                                <div style="position:relative">
                                    ...
                                    <div class="table_content" style="width:120px;">
                                        <?php
                                            ...
                                            echo '<select name="gol_'.$k.'" class="form-control">';

                                            echo '  <option value="1" '.$sel_gol1.'>Air</option>';
                                            echo '  <option value="2" '.$sel_gol2.'>BBM</option>';
                                            echo '  <option value="3" '.$sel_gol3.'>C</option>';
                                            echo '  <option value="4" '.$sel_gol4.'>Elektrikal</option>';
                                            echo '  <option value="5" '.$sel_gol5.'>Fee</option>';
                                            echo '  <option value="6" '.$sel_gol6.'>IMB</option>';
                                            echo '  <option value="7" '.$sel_gol7.'>Jaga</option>';
                                            echo '  <option value="8" '.$sel_gol8.'>Rekening</option>';
                                            echo '  <option value="9" '.$sel_gol9.'>Sewa</option>';
                                    ...
                                    <div class="table_content" style="width:50px;">
                                        <a href="lain_lain.php?edit=<?php echo $edit;?>&del=<?php echo $g['id'];?>" onclick="return confirm('Are you sure?')">
            ...
            ?>
                ...
                <div style="position:relative; margin-bottom:20px; font-weight:bold; font-size:20px">LAIN LAIN</div>
                ...
                <table class="table table-hover">
                    ...
                    echo '<tr>
                            ...
                            <td><a href="lain_lain.php?edit='.$r['project_id'].'"><button type="button" class="btn btn-primary">OPEN</button></td></a>
                          </tr>';
                    $i++;
                    }
                    ?>
